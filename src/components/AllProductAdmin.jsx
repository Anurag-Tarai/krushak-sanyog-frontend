...
const AllProductAdmin = () => {
  ...
  const farmerId = localStorage.getItem("adminid");
    ...
    const fetchProducts = async () => {
      try {
        ...
        const filteredProducts = response.data.filter(
          (product) => product.farmer_id == farmerId
        );

        const sortedProducts = filteredProducts.sort((a, b) => b.productId - a.productId);
  ...
  const handleViewDetails = (productId) => {
    navigate(`/farmer/product-details/${productId}`);
  };
            ...
            >
              ...
              <div className="p-4 text-left">
                ...
                <p className="text-green-800 font-semibold mt-2">
                  Available Quantity: {product.price} kg
                </p>
                <div className="mt-4 flex justify-between">
                  <button
                    className="bg-gradient-to-r from-blue-400 to-blue-500 hover:from-blue-500 hover:to-blue-600 text-white py-2 px-4 rounded-lg font-semibold shadow-md hover:shadow-lg transition-all"
